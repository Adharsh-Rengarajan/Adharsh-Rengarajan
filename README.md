# Hi there, I'm Adharsh Rengarajan 

Welcome to my GitHub profile! I'm currently pursuing a Master of Science in Software Engineering Systems at Northeastern University, Boston. I am passionate about building robust software solutions and have hands-on experience in full-stack development, software testing, and AI/ML integration.

![Profile Views](https://komarev.com/ghpvc/?username=Adharsh-Rengarajan&color=blue)

You can reach me at: rengarajan.ad@northeastern.edu

---

## Education

**Northeastern University, Boston, MA**  
*Master of Science in Software Engineering Systems*  
Expected Graduation: Dec 2026  
- **Relevant Coursework:** Object-Oriented Design, Web Design and User Experience Engineering, Program Structures and Algorithms, Enterprise Software Design.

**K. Ramakrishnan College of Engineering, Trichy, TN**  
*Bachelor of Engineering in Computer Science and Engineering*  
2020 – 2024  
- **Relevant Coursework:** Object-Oriented Programming, Data Structures and Algorithms, Cloud Computing, Artificial Intelligence, Machine Learning, Computer Networks, Operating Systems.

---

## Technical Skills

- **Languages:** Python, Java, JavaScript, TypeScript, HTML5, CSS3
- **Frameworks/Libraries:** React, Redux, Flask, Spring MVC, Servlets, JDBC, Spring Boot, Express.js, Node.js, Gradio
- **Databases & Cloud:** SQL, MongoDB, Firebase
- **Developer Tools:** GitHub, Postman, VS Code, LM Studio
- **Concepts:** Software Engineering, Full-Stack Web Development, RESTful APIs, Version Control (Git), Prompt Engineering, AI/ML Integration

---

## Experience

**Amalgam Rx**  
*Software Testing Intern*  
_Remote | Oct 2023 – Feb 2024_  
- Completed weekly Agile sprint cycles, maintaining thorough documentation and progress reporting.
- Executed comprehensive testing across various software models, enhancing system reliability and performance.
- Led L1 testing to ensure robust functionality and strict adherence to specifications.
- Authored detailed reports documenting performance metrics and improvements.
- Collaborated with cross-functional teams to refine prompts in line with evolving project needs.
- Engineered a Gradio-based automation application, significantly boosting testing efficiency.

---

## Contact

- **Email:** rengarajan.ad@northeastern.edu

Feel free to explore my repositories and connect with me!

---
